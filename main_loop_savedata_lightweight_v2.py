...
from gaze_prediction_v2 import getArch, prediction, get_transformations, draw_annotated_frame
...
def parse_args():
    ...
    parser.add_argument("--output_dir", type=str, default="experiment_data_par"+str(participant_num)+"_sc"+str(scenario_num),
...
def main():
    ...
    with open(csv_path, 'w', newline='') as csv_file:
        ...
        csv_writer.writerow(['participant_num','scenario_num','frame_name', 'timestamp', 'aoi', 'pitch', 'yaw',
                            'smoothed_pitch', 'smoothed_yaw', 'reading_state', 'page_number','elapsed_time'])
    ...
    while i < len(text_list):
        ...
        while True:
            ...
            # Process frame with gaze prediction
            frame, pitch, yaw, face_bbox = prediction(transformations, model, frame)
            ...
            annotated_frame = draw_annotated_frame(
                frame,
                face_bbox,
                (pitch, yaw),                          # Original gaze
                (smoothed_pitch, smoothed_yaw),        # Smoothed gaze
                current_aoi,
                reading_state,
                i
            )
            ...
            # Save data to CSV with both original and smoothed values
