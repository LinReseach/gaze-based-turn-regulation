   ...
   # height, width, layers = frame.shape
    ...
    while i < len(text_list):
        ...
        if i==7:
            ...
            time.sleep(2)#2.5
        # please note that this done not mean wait 3s after speech finishing--this code will somehow run the connect.say first then time.sleep.
        # but here when computer give request to speech, the robot will run speech function this may happen together with
        #time.sleep
        connect.tablet(url_list[i])# if the robot not show the web but just say the website,please check if camera id and res is correct
        time.sleep(1)
        ...
        while True:
            ...
            if (pitch, yaw) != (None, None):
                #smoothed_pitch, smoothed_yaw = ( pitch, yaw)
                ...
                if not is_reading:
                    ...
                    if shift_detected:
                        ...
                        end_page = time.time()
                        print(end_page - start_page, len(gaze_data))
                        break
